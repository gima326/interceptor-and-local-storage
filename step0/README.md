...
* [`src/local_storage/`](src/local_storage/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/local_storage/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `local-storage` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/local_storage/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns local-storage.example
  (:require [local-storage.config :as config])
